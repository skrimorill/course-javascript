...
const server = http.createServer(async (req, res) => {
  try {
    if (/\/photos\/.+\.png/.test(req.url)) {
      ...
      const fileBackPath = path.resolve(__dirname, '../no-photo.png');
      const filePath = path.resolve(__dirname, '../photos', imageName);

      if (fs.existsSync(filePath)) {
        return fs.createReaderStream(filePath).pipe(res);
      } else {
        return fs.createReaderStream(fileBackPath).pipe(res);
      }
